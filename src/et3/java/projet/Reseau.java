...
public class Reseau {
	...
	public void ajoutSeries (String serieTitle, Document doc) {
		...
		for(String key: listeSeries.keySet()) {
			if(serieTitle.equals(listeSeries.get(key).getNom()) && listeSeries.get(key) != null) {
	...
	public void trierParAuteurSurname(String authorName) {
		for(String key: listeBibli.keySet()) {
			listeBibli.get(key).trierParAuteurSurname(authorName);
		}
	}
	...
	public void rechercheEAN(String EAN) {
		for(String key: listeBibli.keySet()) {
			listeBibli.get(key).rechercheISBN(EAN);
		}
	}
